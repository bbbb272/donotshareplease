...
const CONFIG = {
  ...
  chatgpt_answer: 1,   // 1: Enable ChatGPT answer generation, 0: Disable
...
/**
 * Gets an answer from ChatGPT based on provided text
 * @param {string} promptText Text to send to ChatGPT
 * @returns {Promise<string>} ChatGPT's answer
 */
async function getAnswerFromChatGPT(promptText) {
  if (!CONFIG.chatgpt_answer) {
    console.log("ChatGPT answer is disabled in feature flags");
    return "ChatGPT answer is disabled in feature flags";
  }

  console.log("Fetching answer from ChatGPT...");
  try {
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "You are a highly accurate DSA and engineering questions answering bot.",
        },
        {
          role: "user",
    ...
    console.log("Answer received from ChatGPT.");
    return answer;
  } catch (error) {
...
async function processSessionScreenshots(session, ctx) {
  ...
  // Only after all text extraction is complete, send the combined text to ChatGPT
  if (CONFIG.chatgpt_answer && allExtractedText) {
    await ctx.reply("✅ All Extracted Text:\n\n" + allExtractedText);
    await ctx.reply("⏳ *Sending all extracted text to ChatGPT...*");

    try {
      const chatGptAnswer = await getAnswerFromChatGPT(allExtractedText);
      combinedResponse += `\n\n${chatGptAnswer}\n\n`;

      // Send the ChatGPT answer immediately as it might be long
      await ctx.reply(`\n\n${chatGptAnswer}`);
    } catch (error) {
      console.error("Error getting answer from ChatGPT:", error);
      combinedResponse += `❌ *ChatGPT Error:*\n${error.message}\n\n`;
      await ctx.reply(`❌ *ChatGPT Error:*\n${error.message}`);
    }
  }
...
bot.on('text', async (ctx) => {
  ...
  if (message.toLowerCase().includes('ss')) {
    ...
    try {
      ...
      // Get ChatGPT answer if OCR was successful
      if (CONFIG.chatgpt_answer && extractedText && extractedText !== "Failed to extract text") {
        ...
        console.log("Sending ChatGPT answer to user.");
